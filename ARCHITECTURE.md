...
**Crawler crashing on unexpected Lark API response fields** (fixed): found via a production CloudWatch log showing `LarkBaseService.getTableFields` failing 92 times in a single crawl run with `Unrecognized field "error"`. Root cause: `glue-lark-base-crawler`'s `BaseResponse`/`BaseResponse.Builder` (the shared parent all four response types - `ListFieldResponse`, `ListRecordsResponse`, `ListAllTableResponse`, `ListAllFolderResponse` - inherit from) lacked `@JsonIgnoreProperties(ignoreUnknown = true)`, unlike the `athena-lark-base` module's equivalent class, which already had it. Whenever Lark's API returned a response containing an unexpected top-level key (observed: `"error"` alongside the normal `code`/`msg`/`data` shape), Jackson rejected the whole response outright instead of ignoring the extra field, which silently broke LOOKUP field type resolution for every field on the affected table (caught by `getLookupType`'s try/catch, so it didn't crash the crawl, but that table's LOOKUP columns lost their resolved types for that run). Fixed by adding the annotation to `BaseResponse`/`BaseResponse.Builder`, matching the connector module.
